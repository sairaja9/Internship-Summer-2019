...
def processTree(root, outFile):
    ...
    for MyList in root['children']:
        ...
        # states need be max sorted (sort high to low best q_value)
        if 'state' in root["name"]:
            # do stuff
            MyList.sort(key=selector, reverse = True)
            for i in MyList:
                print(i['best q_value'])
            print()

        # max action are min sorted (sort low to high best q_value)
        elif 'max' in root['name']:
            MyList.sort(key=selector, reverse = False)
            for i in MyList:
                print(i['best q_value'])
            print()


        # min actions can be ignored

        # sort the items in MyList ; sort by best q_value      

def selector (x):
    return x["best q_value"]

InputFileName = str(input("Enter JSON Filename (no .json): ")) + ".json"
TextOutput = InputFileName + ".txt"
JsonOutput = "sorted_" + InputFileName

inFile = open(InputFileName, "r") #FIXME check this ptr
outFile = open(TextOutput, "w") #FIXME check this ptr
JSONoutFile = open(JsonOutput, "w")
...
json.dump(data, JSONoutFile, sort_keys=True, indent=4)